...
for (i in 1:100){
  query <- list(
    q = c("story site:http://www.fao.org/news/story/"),
    count = 50,
    offset = (i-1)*50)
...
url_list_fao_2 <- url_list_fao[234]
url_list_fao <- url_list_fao[-234]



# number of pages to scrape
npages <- as.integer(length(url_list_fao))
...
write.csv(out, file = "out_fao.csv")
